...
- `itsashikur.com` / `www.itsashikur.com` → `frontend-main`
- `admin.itsashikur.com` → `frontend-admin`
- `api.itsashikur.com` → Django (includes `/admin/` for Django admin)
...
Set `COMPOSE_PROJECT_NAME` in `.env` before the first production deploy and **do not change it** afterward (Docker volume names depend on it). Existing servers using `ashikur-portfolio-prod-*` containers should set `COMPOSE_PROJECT_NAME=ashikur-portfolio-prod`.
