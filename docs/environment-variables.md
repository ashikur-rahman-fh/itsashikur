...
| `BACKEND_SERVER_API_URL` | **Server-side only** (Next.js RSC / sitemap / RSS). Base URL for `fetchPublicBlog*` helpers. In Docker Compose for **frontend-main**, set to `http://backend:8000`. Falls back to `NEXT_PUBLIC_BACKEND_MAIN_API_URL` in production if unset. |
| `BLOG_REVALIDATE_SECRET` | Shared secret for `POST /api/revalidate/blog` on **frontend-main** (on-demand ISR after publish). Optional; admin app proxies via `POST /api/revalidate/blog` when `FRONTEND_MAIN_URL` is set. |
| `FRONTEND_MAIN_URL` | Public **frontend-main** origin (e.g. `https://www.itsashikur.com`) for admin revalidation proxy. |
...
| Variable               | Purpose                                                                 |
| ---------------------- | ----------------------------------------------------------------------- |
| `COMPOSE_PROJECT_NAME` | Compose project name (container prefix, e.g. `ashikur-portfolio-prod`). **Do not change** after the first deploy or Postgres/Redis volumes will not match. |
