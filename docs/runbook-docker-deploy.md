...
| ------------- | --------------------- | --------------------- |
| `DOCKER_REPO` | `myorg/my-ashikur-portfolio` | `myorg/my-ashikur-portfolio` |
...
| `DEPLOY_PATH` | `/opt/ashikur-portfolio` | `/opt/ashikur-portfolio` |
| `COMPOSE_FILE` | `infra/docker/compose/docker-compose.prod.yml` | same |
| `BACKUP_DIR` | `/var/backups/ashikur-portfolio` | `/var/backups/ashikur-portfolio` |
...
| `NEXT_PUBLIC_BACKEND_MAIN_API_URL` | Yes — **public API URL in the browser** (e.g. `https://api.itsashikur.com`), not `http://backend:8000` |
...
- `DOCKER_REPO` = `myorg/my-ashikur-portfolio`
...
| App | Image name |
| --- | ---------- |
| Backend | `myorg/my-ashikur-portfolio-backend` |
| Main frontend | `myorg/my-ashikur-portfolio-frontend-main` |
| Admin frontend | `myorg/my-ashikur-portfolio-frontend-admin` |
| Nginx | `myorg/my-ashikur-portfolio-nginx` |
...
```text
myorg/my-ashikur-portfolio-backend:staging-260515-1
myorg/my-ashikur-portfolio-backend:staging-latest
```
...
**Important:** The **Deploy to VM** job runs on the server you set in `VM_HOST` (over SSH). Your laptop can have `infra/env/prod/.env`, but the deploy will still fail until that file exists **inside `DEPLOY_PATH` on the VM** (e.g. `/home/.../deployment/ashikur-portfolio/infra/env/prod/.env`).

1. Install **Docker** and **Docker Compose v2**.
2. Add the deploy user to the `docker` group.
3. **Clone this repository** to `DEPLOY_PATH` (e.g. `/opt/ashikur-portfolio`).
...
6. Set registry deploy keys in the same `.env` for manual deploy: `DOCKER_REPO`, `BRANCH_SLUG`, `DOCKER_USERNAME`, `DOCKER_TOKEN`, `BACKUP_DIR`, `COMPOSE_PROJECT_NAME` (keep `ashikur-portfolio-prod` if containers already use that prefix), `APP_DOMAIN` / `API_HOST` (mirror GitHub Environment variables).
...
9. Create backup directory (or let the deploy script create it with mode `700`):
   ...
   ```bash
   sudo mkdir -p /var/backups/ashikur-portfolio && sudo chown $USER:$USER /var/backups/ashikur-portfolio
   ```
...
| Backup directory | `/var/backups/ashikur-portfolio/` (or `BACKUP_DIR`) |
...
```bash
cd /opt/ashikur-portfolio   # your DEPLOY_PATH
