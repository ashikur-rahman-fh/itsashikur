...
Ashikur Portfolio is a **multi-container** system orchestrated by Docker Compose, with a **pnpm monorepo** for TypeScript frontends and a standalone Django backend.
...
- Call the backend using shared helpers from `@ashikur-portfolio/shared`
- Are built as **Next.js standalone** outputs in production images

### Shared package (`packages/shared`)

Workspace package `@ashikur-portfolio/shared` provides:
...
- **Shared UI** (`packages/shared/src/ui/`) — **Ashikur Portfolio** theme (muted slate primary, soft neutrals, Inter + JetBrains Mono, light/dark). Tailwind v4, shadcn primitives, Basecoat CSS. Import via `@ashikur-portfolio/shared/ui`. See [`ui-system.md`](ui-system.md).
- Hooks (`useApi`)
...
Both frontends import `@ashikur-portfolio/shared/ui/styles/globals.css` in root layout and wrap content with `ThemeProvider`.

#### Shared API client

```text
packages/shared/src/api/
  index.ts              # public barrel (@ashikur-portfolio/shared/api)
  clients/
...
- **Single Axios dependency** lives in `@ashikur-portfolio/shared`; both frontends consume clients from here.
...
- **Prod**: HTTPS-only routing by hostname (`itsashikur.com`, `admin.itsashikur.com`, `api.itsashikur.com` in the sample config).
